class Animation_Train_OutAndIn extends Animation {
  public Animation_Train_OutAndIn(int zoneID, int layerIndex) {
  ...
  private Frame generateFrame() {
    ...
    if (this.frameCount <= this.frameSize()) {
      ...
      for (int i = 0; i < this.frameSize() - (this.frameCount - this.frameSize()); i++) {
        frame.setPixel(i, pixel);
      }
    }
